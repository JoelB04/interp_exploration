...
±plus-minus 10% band across the entire range the real data occupies. Note also that at M=160
the curve crosses $\hat D/D = 1$ near D=15, two opposite biases cancelling. A ratio near 1 is not evidence of a good estimate.
...
The right panel is what the null looks like: the true taxonomy sits in the
