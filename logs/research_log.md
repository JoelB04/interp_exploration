...
================================================================================
PROJECT 2 -- HIERARCHICAL GEOMETRY OF MISALIGNMENT CONCEPTS
================================================================================

Started 2026-08-27 after the transfer project closed. Question: does the model
organise misalignment-related concepts hierarchically, and where in depth does
that structure appear?

Two arms. Synthetic hierarchical manifolds with known ground truth, to calibrate
the estimators and bound what this design could detect; and SALAD-Bench prompts
through Qwen2.5-1.5B, geometry per layer.
...
## 2026-08-29 -- session 6: MMLU as a second taxonomy

**Question.** Does representational geometry reflect an externally-given
category hierarchy in general, or is the SALAD result specific to harm?

**Prediction (write this before the analysis runs).** *(to fill in)*

**Setup.** src/mmlu.py, 22_extract_mmlu.py. 13 subjects across MMLU's own
published 4-way grouping, branching [4,3,3,3], M=200 -- forced, since only 3 of
57 subjects have >=640 rows. Subjects chosen for question length near SALAD's
77-102 chars; achieved 76-129, mean 92. Question stems only, no answer choices.
Modes raw and request; not chat, which is compromised.

SALAD to be subsampled to M=200 for the matched comparison. Compare z-scores
against each dataset's own matched null, since branching differs (15
within-parent pairs against SALAD's 12).

**Result.** *(pending -- extraction running)*
...
- within_scale recalibration and the synthetic null band / power sweep at the
  real regime (spread/separation ~2-5, not 0.018). Without this there is no
  bound on what the design could have detected.
- MMLU comparison.
- README and write-up.
