# Archive -- truth-probe transfer project (closed 2026-08-27)

A separate, earlier project kept here so the main research log covers one thing.
Not part of the manifold-geometry work; read `research_log.md` for that.

It asked whether probe transfer across truth datasets was predictable from
geometry. Seven rounds, five retractions, no surviving claim that reading
internals beats a free baseline out of distribution. Closed because the
correlational framing needed many independent datasets and only ~5 families
existed -- a structural cap rather than a fixable problem. That is what motivated
the pivot to manifolds as the unit of analysis.

The code is in git at `73f06af`:
